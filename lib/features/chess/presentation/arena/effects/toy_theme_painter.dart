...
class ToyPiecePainter extends CustomPainter {
  ...
  Path _getToyPath(String type, Offset center, double radius) {
    switch (type) {
      case 'K':
        return _toyKingPath(center, radius);
      case 'Q':
        return _toyQueenPath(center, radius);
      case 'B':
        return _toyBishopPath(center, radius);
      case 'N':
        return _toyKnightPath(center, radius);
      case 'R':
        return _toyRookPath(center, radius);
      default:
        return _toyPawnPath(center, radius);
    }
  }
  ...
  Path _toyKnightPath(Offset center, double radius) {
    return Path()
      ...
      ..addRRect(
        RRect.fromRectAndRadius(
          Rect.fromCenter(
            center: center - Offset(radius * 0.1, 0),
            width: radius * 0.6,
            height: radius * 1.0,
          ),
          const Radius.circular(12),
      ...
      ..addRRect(
        RRect.fromRectAndRadius(
          Rect.fromCenter(
            center: center - Offset(radius * 0.4, radius * 0.3),
            width: radius * 0.4,
            height: radius * 0.3,
          ),
