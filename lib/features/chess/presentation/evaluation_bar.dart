...
class EvaluationBar extends StatelessWidget {
  ...
  List<Widget> _buildSegments(double score, bool isWhiteWinning, bool isBlackWinning) {
    ...
    if (orientation == Axis.horizontal) {
      if (isWhiteWinning) {
        segments.add(Expanded(flex: (score * 100).round(), child: Container(color: Colors.white)));
        segments.add(Expanded(flex: ((1.0 - score) * 100).round(), child: Container(color: Colors.black38)));
      } else {
        segments.add(Expanded(flex: (score * 100).round(), child: Container(color: Colors.black38)));
        segments.add(Expanded(flex: ((1.0 - score) * 100).round(), child: Container(color: Colors.black)));
      }
    } else {
